...
@interface SPCitiesDataTool ()
@property (nonatomic,strong) NSMutableArray * dataArray;
@property (nonatomic, strong)  FMDatabase *fmdb;
@end

@implementation SPCitiesDataTool

static SPCitiesDataTool *shareInstance = nil;

#pragma mark - Singleton
+ (SPCitiesDataTool *)sharedManager
{
    @synchronized (self) {
...
- (void)requestGetData{
    NSLog(@"isTableOk = %d",[self isTableOK]);
    if ([self isTableOK]) {
        return;
    }
    ...
    for (NSDictionary * dict in jsonObjectArray) {
        SPAddressItem * item = [[SPAddressItem alloc] initWithDict:dict];
...
- (void)insertRecords{
    ...
    if ([self.fmdb open] && [self.fmdb beginTransaction]) {
        ...
        @try
        {
            for (SPAddressItem * item in self.dataArray) {
...
- (NSMutableArray *)queryAllProvince
{
    if ([self.fmdb  open]) {
        ...
        //'code','sheng','di','xian','name', 'level'
        while ([result next]) {
            SPAddressItem *model = [[SPAddressItem alloc] init];
